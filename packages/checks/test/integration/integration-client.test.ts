...
import {
  ...
  name as packageName
} from 'rootverse+checks:package.json';
...
const debug = createDebugLogger({ namespace: 'checks' }).extend(TEST_IDENTIFIER);
...
beforeAll(async () => {
  await ensurePackageHasBeenBuilt(
    toDirname(toAbsolutePath(require.resolve('rootverse+checks:package.json'))),
    packageName,
    packageExports
  );
});
